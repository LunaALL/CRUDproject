...
public class MemberDAO {
	...
	public Memberinfo selectByEmail(String email) {
		List<Memberinfo> results = jdbcTemplate.query(new PreparedStatementCreator() {
			@Override
	...
	public Memberinfo selectByName(String name) {
		List<Memberinfo> results = jdbcTemplate.query("select * from member where NAME =? ", new MemberinfoRowMapper(),
				name);
	...
	public void insert(Memberinfo member) {
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				// TODO Auto-generated method stub
				PreparedStatement pstt = con.prepareStatement(
	...
	public boolean isAdmin(String email) {
		List<Memberinfo> results = jdbcTemplate.query("select * from member where admin=1 && EMAIL=?",
